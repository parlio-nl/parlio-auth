package nl.parlio.auth.login.web
...
class AuthorizationController(
    ...
    fun authorizationFailed(model: Model, request: HttpServletRequest): String {
        ...
        if (StringUtils.hasText(errorCode)) {
            // @formatter:off
            model.addAttribute("error", OAuth2Error(
                    errorCode,
    ...
    fun clientCredentialsGrant(model: Model, restTemplate: RestTemplate): String {
        val messages = webClient
            .get()
            .uri(messagesBaseUri)
